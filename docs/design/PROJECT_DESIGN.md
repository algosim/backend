...
## 5. Database Schema for Auth Module

The **Auth Module** will use a **PostgreSQL** database.
...
| password_hash | TEXT (Nullable) | Hashed password (if using password login) |
| oauth_provider | TEXT (Nullable) | `google` if OAuth-based login |
| oauth_id      | TEXT (Nullable) | Google OAuth user ID          |
